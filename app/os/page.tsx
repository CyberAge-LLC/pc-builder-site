...
export default function OSTable() {
  ...
  const fetchData = async () => {
    ...
    const { data: rows, error } = await supabase
      .from('os') // Ensure the table name is correct
      .select('*', { count: 'exact' });
    ...
    const filteredRows = rows?.filter(row =>
      ...
      !isNaN(parseFloat(row.max_memory))
    ) || [];
          ...
          <table style={{ border: '1px solid black', width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ display: 'flex', textAlign: 'left', backgroundColor: '#2a2a2a' }}>
                {['name', 'price', 'mode', 'max_memory'].map((key) => (
                  <th
                    key={key}
                    ...
                    style={{
                      ...
                      textAlign: 'center',
                      color: 'white',
                      cursor: key === 'name' || key === 'mode' ? 'default' : 'pointer',
                    }}
                  >
                  ...
                  <tr key={row.id}>
                    <td colSpan={8} style={{ padding: '0' }}>
                      ...
                      >
                        ...
                        <div style={{ flex: '1 1 10%' }}>{row.mode}</div>
