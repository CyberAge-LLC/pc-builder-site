...
export default function CaseTable() {
  ...
  const fetchData = async () => {
    ...
    const { data: rows, error } = await supabase
      .from('case') // Ensure the table name is correct
      .select('*', { count: 'exact' });
  ...
  const handleSort = (key: keyof TableRow) => {
    // Only allow sorting for numeric fields and disable sorting for non-numeric fields like name, type, color, psu, side_panel
    if (key === 'name' || key === 'type' || key === 'color' || key === 'psu' || key === 'side_panel') return;
  ...
  const sortData = (key: keyof TableRow, direction: 'ascending' | 'descending') => {
    const sorted = [...data].sort((a, b) => {
      ...
      if (key === 'price' || key === 'external_volume' || key === 'internal_35_bays') {
          ...
          <table style={{ border: '1px solid black', width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ display: 'flex', textAlign: 'left', backgroundColor: '#2a2a2a' }}>
                {['name', 'price', 'type', 'color', 'psu', 'side_panel', 'external_volume', 'internal_35_bays'].map((key) => (
                  <th
                    key={key}
                    ...
                    style={{
                      ...
                      textAlign: 'center',
                      color: 'white',
                      cursor: (key === 'name' || key === 'type' || key === 'color' || key === 'psu' || key === 'side_panel') ? 'default' : 'pointer',
                    }}
                  >
                  ...
                  <tr key={row.id}>
                    <td colSpan={8} style={{ padding: '0' }}>
                      ...
                      >
                        ...
                        <div style={{ flex: '1 1 10%' }}>{row.psu}</div>
                        <div style={{ flex: '1 1 10%' }}>{row.side_panel}</div>
                        <div style={{ flex: '1 1 10%' }}>{row.external_volume}</div>
                        <div style={{ flex: '1 1 10%' }}>{row.internal_35_bays}</div>
                      </button>
                    </td>
                  </tr>
                ))
